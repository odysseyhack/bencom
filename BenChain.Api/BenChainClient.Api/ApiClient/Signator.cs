...
namespace BenChainClient.Api
{
    ...
    public partial class Signator : IServiceOperations<ApiClient>, ISignator
    {
        ...
        /// A response object containing the response body and response headers.
        /// </return>
        public async Task<HttpOperationResponse<IList<SignatoryModel>>> GetAllByParticipantWithHttpMessagesAsync(Guid participantId, int status, Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            // Tracing
            bool _shouldTrace = ServiceClientTracing.IsEnabled;
            string _invocationId = null;
        ...
        /// A response object containing the response body and response headers.
        /// </return>
        public async Task<HttpOperationResponse<IList<SignatoryModel>>> GetAllNotOpenByParticipantWithHttpMessagesAsync(Guid participantId, Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            // Tracing
            bool _shouldTrace = ServiceClientTracing.IsEnabled;
            string _invocationId = null;
            if (_shouldTrace)
            {
                ...
                tracingParameters.Add("participantId", participantId);
                tracingParameters.Add("cancellationToken", cancellationToken);
                ServiceClientTracing.Enter(_invocationId, this, "GetAllNotOpenByParticipant", tracingParameters);
            }
            // Construct URL
            var _baseUrl = this.Client.BaseUri.AbsoluteUri;
            var _url = new Uri(new Uri(_baseUrl + (_baseUrl.EndsWith("/") ? "" : "/")), "api/signator/bystatusnotopen").ToString();
            List<string> _queryParameters = new List<string>();
            _queryParameters.Add(string.Format("participantId={0}", Uri.EscapeDataString(SafeJsonConvert.SerializeObject(participantId, this.Client.SerializationSettings).Trim('"'))));
            if (_queryParameters.Count > 0)
            {
